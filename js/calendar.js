...
export const FUTURE_DATE_MESSAGE='この日はまだ未来の日付です。記録は当日以降に登録できます。';
export const isFutureDate=(date,today=dateKey())=>date>today;
export function assertRecordableDate(date,today=dateKey()){
  if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||dateKey(parseDate(date))!==date)throw new Error('日付を確認してください');
  if(isFutureDate(date,today))throw new Error(FUTURE_DATE_MESSAGE);
}
...
export function calendarDayState(date,count,appStartedAt,today=dateKey()){
  if(count>0)return 'flower';
  if(date<appStartedAt)return 'empty';
  return 'sprout';
}
