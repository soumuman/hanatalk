...
const greenery=body=>`<g class="flower-foliage">${body}</g>`;
...
export function renderUme(progress){
  ...
  return greenery(branch)+ring(5,progress,petal)+circle(50,43,6,light)+stamens(10,7)+circle(50,43,2.5,'#d5ad56');
}
export function renderSuisen(progress){
  ...
  return greenery(stem()+foliage)+ring(6,progress,petal,15)+trumpet;
}
export function renderSakura(progress){
  ...
  return greenery(branch)+ring(5,progress,petal)+circle(50,43,5,light)+stamens(7,6,'#bd7993');
}
export function renderTulip(progress){
  ...
  return greenery(stem()+foliage)+back+center+front;
}
export function renderNemophila(progress){
  ...
  return greenery(stem()+leaves())+ring(5,progress,petal,12)+circle(50,43,5,'#faf7e9')+stamens(5,4,'#596f86');
}
...
export function renderHydrangea(progress){
  ...
  const positions=[[50,43],...[{n:6,r:12},{n:12,r:24}].flatMap(({n,r})=>Array.from({length:n},(_,i)=>{
    const angle=-Math.PI/2+i*Math.PI*2/n;
    return [+(50+Math.cos(angle)*r).toFixed(2),+(43+Math.sin(angle)*r).toFixed(2)];
  }))];
  return greenery(stem()+foliage)+positions.slice(0,visible(progress,positions.length)).map(([x,y],i)=>floret(x,y,1.22,[pink,light,'#b5b9da'][i%3])).join('')+(!progress?floret(50,43,.8,'#cbd7bd'):'');
}
export function renderMorningGlory(progress){
  ...
  return greenery(vine+foliage)+trumpet+`<g transform="translate(50 43) scale(${scale.toFixed(3)} ${(0.68+.32*p).toFixed(3)}) translate(-50 -43)">${mouth}${star}${circle(50,44,3,'#ead3aa')}</g>`;
}
export function renderSunflower(progress){
  ...
  return greenery(stem()+foliage)+ring(20,progress,petal)+circle(50,43,15,'#876444')+circle(50,43,11,'#715439')+seeds;
}
export function renderCosmos(progress){
  ...
  return greenery(stem()+foliage)+ring(8,progress,petal)+circle(50,43,7,'#ddbd61')+stamens(7,4,'#ad944b');
}
export function renderOsmanthus(progress){
  ...
  return greenery(branch+foliage)+positions.slice(0,visible(progress,positions.length)).map(([x,y],i)=>floret(x,y,.90,i%3===0?light:pink,'#b58341')).join('')+(!progress?circle(48,45,3,'#bdcbab'):'');
}
export function renderChrysanthemum(progress){
  ...
  return greenery(stem()+foliage)+ring(16,progress,outer)+ring(12,progress,middle,15)+ring(8,progress,inner,25)+circle(50,43,4,'#d6ae57');
}
export function renderPoinsettia(progress){
  ...
  return greenery(stem()+lower)+outer+inner+circle(50,43,5,'#a6a962')+stamens(5,3,'#e4c773');
}
