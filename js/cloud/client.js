...
export async function sendLink(email){const {error}=await client.auth.signInWithOtp(emailLinkRequest(email,appURL));if(error)throw error;}
export function signInGoogle(){return beginGoogleLogin({client,projectURL:config.url,key:config.key,origin:appURL,request:timedFetch,navigate:url=>location.assign(url)});}
