...
function addPeople(first=firstRegistration){
  app.innerHTML=`<section class="add-people">${first?'':'<a class="back" href="#settings">‹ 人物・設定</a>'}${first?'<div class="info-box"><h2>すでに記録がある方</h2><p>ホーム画面版では、元のブラウザの記録は自動では表示されません。</p><a class="primary wide" href="#settings">クラウドの記録を使う</a><p class="small">先に元のブラウザでログインし、記録をクラウドへ引き継いでください。</p></div>':''}<h1>${first?'人物を登録':'人物を追加'}</h1><p>基本のカードを用意しました。名前を入力して追加もできます。</p><div class="choices">${suggestions.map(([name],i)=>{
