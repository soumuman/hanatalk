import {cloudPanel,paintCloud,bootCloud,cloudEntryState,cloudEntryError} from './cloud/ui.js';
...
function onboarding(){
  ...
  app.innerHTML=`<section class="title-welcome" aria-labelledby="welcome-title"><div class="welcome-sprout">${sproutSVG()}</div><h1 id="welcome-title">HanaTalk</h1><p class="welcome-tagline">今日、誰としゃべった？</p><p class="welcome-description">話した人を記録すると、<br>少しずつ花が咲いていきます。</p><a class="primary welcome-start" href="#register">はじめる</a>${standalone?'':'<button class="text-link welcome-install" data-action="install-guide">ホーム画面に追加すると便利です</button>'}</section>`;
}
function installGuide(){
  const ios=/iPad|iPhone|iPod/.test(navigator.userAgent)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1);
  dialog(`<h2>ホーム画面に追加</h2><p>HanaTalkをホーム画面に追加すると、<br>アプリのようにすぐ開けます。</p><p>${ios?'共有ボタン →「ホーム画面に追加」':'ブラウザのメニューから「アプリをインストール」または「ホーム画面に追加」を選べます。'}</p><div class="dialog-actions"><button data-close>閉じる</button></div>`);
}
function calendar(){
...
function day(){
  document.body.classList.add('entry-view');
  ...
  app.innerHTML=`<section class="entry"><div class="entry-date"><a href="#calendar" class="back">‹ 戻る</a><p class="date-label">${formatDate(date)}</p></div><div id="flower-stage" class="flower-stage">${flowerSVG(date,total,{large:true})}<p class="count">${date===dateKey()?'今日':'この日'} ${total}人と話しました</p><p class="bloom-message">${total>=5?'今日の花が咲きました 🌸':'ひとことのつながりを、少しずつ。'}</p></div><p class="entry-prompt">話した相手をタップしてください。</p><div class="people-grid">${shown.map(p=>entryCard(p,selected.get(p.id))).join('')}</div><a class="add-person" href="#settings">＋ 相手を追加・編集する</a>${dayLogs.some(l=>!people.find(p=>p.id===logTargetId(l))?.isActive)?'<p class="small">一覧から外した相手の記録も、花に含めています。</p>':''}<p class="saved">✓ 操作するたびに自動保存します</p><p class="small">カードをもう一度押すと、その相手の選択を取り消せます。</p></section>`;
}
function settings(){app.innerHTML=`<section class="settings"><a href="#calendar" class="back">‹ カレンダー</a><span class="eyebrow">あなたのつながり</span><h1>人物・設定</h1><p>「…」で設定、つまみで並び替えできます。</p><div class="person-settings-grid">${people.filter(p=>p.isActive).map(settingsCard).join('')}</div><div class="choices setting-candidates">${suggestions.map(([name],i)=>people.some(p=>p.displayName===name)?'':`<button data-quick-person="${i}">${esc(name)}<small>＋ 追加</small></button>`).join('')}</div><form id="quick-add"><label for="quick-name">名前を入力して追加</label><div class="input-row"><input id="quick-name" name="name" maxlength="40" placeholder="鈴木さん、カラオケ仲間 など" autocomplete="off" required><button type="submit">追加</button></div></form><p class="small">追加・並び替えは自動で保存します。</p><div class="info-box sound-setting"><h2>効果音</h2><button data-action="sound" role="switch" aria-checked="${soundEnabled}" aria-label="効果音">効果音 ${soundEnabled?'ON':'OFF'}</button><p>花が色づくときに、柔らかな音を鳴らします。</p></div>${cloudPanel()}<div class="info-box"><h2>会話内容は保存しません</h2><p>保存するのは相手の名前・種類・関係カテゴリ・話した日・人数と、アプリの管理情報です。会話内容や音声、位置情報は保存しません。</p><p>端末内保存のみでも利用できます。クラウド同期はログインして有効にした場合だけ使います。</p></div><details class="guide"><summary>この花カレンダーについて</summary><p>ひとりと声を交わすごとに、その月の花が少しずつ開きます。5人で花が完成し、そのあとは色合いが少し変わります。</p><p>小さな双葉は、これから育つ花のスタート地点です。話さない日も、そのままで大丈夫。</p><p>店員さんへの「お願いします」も記録できます。AI・テレビ・独り言は含めません。複数人をまとめたカードでは、選んだあとに＋－で人数を調整できます。個別カードで選んだ人は、その人数に含めません。</p></details><details class="guide"><summary>ホーム画面に追加する</summary><p>iPhoneのSafariでこのアプリを開き、共有メニューから「ホーム画面に追加」を選んでください。一度読み込むとオフラインでも使えます。ブラウザとホーム画面版の端末内データは別の保存先です。先に元のブラウザでクラウド同期を有効にし、ホーム画面版では同じアカウントで「クラウド側データを使う」を選んでください。</p></details><details class="guide archived-cards"><summary>一覧から外したカード</summary><p class="small">戻しても、過去の記録はそのままです。</p>${people.filter(p=>!p.isActive).map(p=>`<div class="restore-row"><span>${esc(p.displayName)}</span><button data-restore="${p.id}">一覧に戻す</button></div>`).join('')||'<p class="small">一覧から外したカードはありません。</p>'}</details><p class="small">HanaTalk · バージョン ${APP_VERSION}</p><button data-action="update-app">最新版を読み込む</button></section>`;paintCloud();}
function addPeople(first=firstRegistration){
  app.innerHTML=`<section class="add-people">${first?'':'<a class="back" href="#settings">‹ 人物・設定</a>'}<h1>${first?'人物を登録':'人物を追加'}</h1><p>使う相手を整えましょう。「…」で名前変更・一覧から外す、つまみで並び替えができます。</p><div class="person-settings-grid">${people.filter(p=>p.isActive).map(settingsCard).join('')}</div><div class="choices setting-candidates">${suggestions.map(([name],i)=>people.some(p=>p.displayName===name)?'':`<button data-quick-person="${i}">${esc(name)}<small>＋ 追加</small></button>`).join('')}</div><form id="quick-add"><label for="quick-name">名前を入力して追加</label><div class="input-row"><input id="quick-name" name="name" maxlength="40" placeholder="鈴木さん、カラオケ仲間 など" autocomplete="off" required><button type="submit">追加</button></div></form><details class="guide archived-cards"><summary>一覧から外したカード</summary><p class="small">必要になったら、ここから戻せます。</p>${people.filter(p=>!p.isActive).map(p=>`<div class="restore-row"><span>${esc(p.displayName)}</span><button data-restore="${p.id}">一覧に戻す</button></div>`).join('')||'<p class="small">一覧から外したカードはありません。</p>'}</details><p class="small">変更は自動で保存します。使わないカードは外して大丈夫です。</p>${first?'<details class="guide"><summary>すでに記録がある方</summary><button data-restore-login>ログインして記録を引き継ぐ</button></details>':''}${first?button('完了','initialize','primary wide'):'<a class="primary wide" href="#settings">完了</a>'}</section>`;
}
...
async function renderView(){const entry=cloudEntryState();if(entry){app.innerHTML=entry==='loading'?'<section role="status" aria-live="polite"><p>記録を読み込んでいます…</p></section>':`<section><p role="alert">${esc(cloudEntryError())}</p><button data-restore-login>再試行する</button><button data-restore-cancel>端末の記録に戻る</button></section>`;return;}document.body.classList.remove('calendar-view','entry-view','welcome-view');appStartedAt=await db.ensureAppStartedAt();await db.ensureStarterCards();await refresh();if(await db.setting('appVersion')!==APP_VERSION)await db.put('settings',{key:'appVersion',value:APP_VERSION});firstRegistration=!await db.setting('initialized');if(firstRegistration&&location.hash!=='#settings')return location.hash==='#register'?addPeople(true):onboarding();if(location.hash.startsWith('#day/'))day();else if(location.hash==='#add-people')addPeople();else if(location.hash==='#settings')settings();else calendar();}
