...
function addPeople(first=firstRegistration){
  ...
  }).join('')}</div><form id="quick-add"><label for="quick-name">名前を入力して追加</label><div class="input-row"><input id="quick-name" name="name" maxlength="40" placeholder="鈴木さん" autocomplete="off" required><button type="submit">追加</button></div></form><div class="registered-names">${people.filter(p=>p.isActive&&!suggestions.some(([name])=>name===p.displayName)).map(p=>`<span>${esc(p.displayName)} <small>登録済み</small></span>`).join('')}</div><p class="small">カードの種類や名前は、あとから「…」で変更できます。</p>${first?button('完了','initialize','primary wide'):'<a class="primary wide" href="#settings">完了</a>'}</section>`;
}
