...
function day(){
  ...
  const shown=people.filter(p=>p.isActive||selected.has(p.id)),total=totalTalkCount(dayLogs);
  app.innerHTML=`<section class="entry"><a href="#calendar" class="back">‹ カレンダー</a><p class="date-label">${formatDate(date)}</p><h1>今日、誰としゃべった？</h1><p>話した相手をタップしてください。</p><div class="people-grid">${shown.map(p=>entryCard(p,selected.get(p.id))).join('')}</div><a class="add-person" href="#settings">＋ 相手を追加・編集する</a><div id="flower-stage" class="flower-stage">${flowerSVG(date,total,{large:true})}<p class="count">${date===dateKey()?'今日':'この日'} ${total}人と話しました</p><p class="bloom-message">${total>=5?'今日の花が咲きました 🌸':'ひとことのつながりを、少しずつ。'}</p></div><p class="saved">✓ 操作するたびに自動保存します</p><p class="small">カードをもう一度押すと、その相手の選択を取り消せます。</p></section>`;
}
function settings(){app.innerHTML=`<section class="settings"><a href="#calendar" class="back">‹ カレンダー</a><span class="eyebrow">あなたのつながり</span><h1>人物・設定</h1><p>「…」で設定、つまみで並び替えできます。</p><div class="person-settings-grid">${people.map(settingsCard).join('')}</div><div class="choices setting-candidates">${suggestions.map(([name],i)=>people.some(p=>p.displayName===name)?'':`<button data-quick-person="${i}">${esc(name)}<small>＋ 追加</small></button>`).join('')}</div><form id="quick-add"><label for="quick-name">名前を入力して追加</label><div class="input-row"><input id="quick-name" name="name" maxlength="40" placeholder="例：平井さん、長男" autocomplete="off" required><button type="submit">追加</button></div></form><p class="small">追加・並び替えは自動で保存します。</p><div class="info-box sound-setting"><h2>効果音</h2><button data-action="sound" role="switch" aria-checked="${soundEnabled}" aria-label="効果音">効果音 ${soundEnabled?'ON':'OFF'}</button><p>花が色づくときに、柔らかな音を鳴らします。</p></div><div class="info-box"><h2>あなたの端末だけに保存</h2><p>保存するのは相手の名前・種類・関係カテゴリ・話した日・人数と、アプリの管理情報です。会話内容や音声、位置情報は保存しません。</p><p>ブラウザのデータを消すと記録も消えます。機種変更時の引き継ぎには、まだ対応していません。</p></div><details class="guide"><summary>この花カレンダーについて</summary><p>ひとりと声を交わすと、花びらが1枚色づきます。5人で花が咲き、そのあとは色合いが少し変わります。</p><p>小さな双葉は、これから育つ花のスタート地点です。話さない日も、そのままで大丈夫。</p><p>店員さんへの「お願いします」も記録できます。AI・テレビ・独り言は含めません。複数人をまとめたカードでは、選んだあとに＋－で人数を調整できます。個別カードで選んだ人は、その人数に含めません。</p></details><details class="guide"><summary>ホーム画面に追加する</summary><p>iPhoneのSafariでこのアプリを開き、共有メニューから「ホーム画面に追加」を選んでください。一度読み込むとオフラインでも使えます。</p></details><p class="small">今日、誰としゃべった？ · バージョン ${APP_VERSION}</p><button data-action="update-app">最新版を読み込む</button></section>`;}
...
function editPerson(id){
  ...
  el.querySelector('form').onsubmit=async e=>{
    ...
    try{await db.updatePerson(id,{displayName:name,type:form.get('type'),category:form.get('category'),isActive:form.has('active')});el.close();await render();notify('カードを保存しました');}
