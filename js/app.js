...
function settings(){app.innerHTML=`<section class="settings"><a href="#calendar" class="back">‹ カレンダー</a><span class="eyebrow">あなたのつながり</span><h1>人物・設定</h1><p>「…」で設定、つまみで並び替えできます。</p><div class="person-settings-grid">${people.filter(p=>p.isActive).map(settingsCard).join('')}</div><div class="choices setting-candidates">${suggestions.map(([name],i)=>people.some(p=>p.displayName===name)?'':`<button data-quick-person="${i}">${esc(name)}<small>＋ 追加</small></button>`).join('')}</div><form id="quick-add"><label for="quick-name">名前を入力して追加</label><div class="input-row"><input id="quick-name" name="name" maxlength="40" placeholder="鈴木さん" autocomplete="off" required><button type="submit">追加</button></div></form><p class="small">追加・並び替えは自動で保存します。</p><div class="info-box sound-setting"><h2>効果音</h2><button data-action="sound" role="switch" aria-checked="${soundEnabled}" aria-label="効果音">効果音 ${soundEnabled?'ON':'OFF'}</button><p>花が色づくときに、柔らかな音を鳴らします。</p></div>${cloudPanel()}<div class="info-box"><h2>会話内容は保存しません</h2><p>保存するのは相手の名前・種類・関係カテゴリ・話した日・人数と、アプリの管理情報です。会話内容や音声、位置情報は保存しません。</p><p>端末内保存のみでも利用できます。クラウド同期はログインして有効にした場合だけ使います。</p></div><details class="guide"><summary>この花カレンダーについて</summary><p>ひとりと声を交わすごとに、その月の花が少しずつ開きます。5人で花が完成し、そのあとは色合いが少し変わります。</p><p>小さな双葉は、これから育つ花のスタート地点です。話さない日も、そのままで大丈夫。</p><p>店員さんへの「お願いします」も記録できます。AI・テレビ・独り言は含めません。複数人をまとめたカードでは、選んだあとに＋－で人数を調整できます。個別カードで選んだ人は、その人数に含めません。</p></details><details class="guide"><summary>ホーム画面に追加する</summary><p>iPhoneのSafariでこのアプリを開き、共有メニューから「ホーム画面に追加」を選んでください。一度読み込むとオフラインでも使えます。ブラウザとホーム画面版の端末内データは別の保存先です。先に元のブラウザでクラウド同期を有効にし、ホーム画面版では同じアカウントで「クラウド側データを使う」を選んでください。</p></details><details class="guide archived-cards"><summary>一覧から外したカード</summary><p class="small">戻しても、過去の記録はそのままです。</p>${people.filter(p=>!p.isActive).map(p=>`<div class="restore-row"><span>${esc(p.displayName)}</span><button data-restore="${p.id}">一覧に戻す</button></div>`).join('')||'<p class="small">一覧から外したカードはありません。</p>'}</details><p class="small">今日、誰としゃべった？ · バージョン ${APP_VERSION}</p><button data-action="update-app">最新版を読み込む</button></section>`;paintCloud();}
function addPeople(first=firstRegistration){
  app.innerHTML=`<section class="add-people">${first?'':'<a class="back" href="#settings">‹ 人物・設定</a>'}<h1>${first?'人物を登録':'人物を追加'}</h1><p>使う相手を整えましょう。「…」で名前変更・一覧から外す、つまみで並び替えができます。</p><div class="person-settings-grid">${people.filter(p=>p.isActive).map(settingsCard).join('')}</div><div class="choices setting-candidates">${suggestions.map(([name],i)=>people.some(p=>p.displayName===name)?'':`<button data-quick-person="${i}">${esc(name)}<small>＋ 追加</small></button>`).join('')}</div><form id="quick-add"><label for="quick-name">名前を入力して追加</label><div class="input-row"><input id="quick-name" name="name" maxlength="40" placeholder="鈴木さん" autocomplete="off" required><button type="submit">追加</button></div></form><details class="guide archived-cards"><summary>一覧から外したカード</summary><p class="small">必要になったら、ここから戻せます。</p>${people.filter(p=>!p.isActive).map(p=>`<div class="restore-row"><span>${esc(p.displayName)}</span><button data-restore="${p.id}">一覧に戻す</button></div>`).join('')||'<p class="small">一覧から外したカードはありません。</p>'}</details><p class="small">変更は自動で保存します。使わないカードは外して大丈夫です。</p>${first?button('完了','initialize','primary wide'):'<a class="primary wide" href="#settings">完了</a>'}</section>`;
}
