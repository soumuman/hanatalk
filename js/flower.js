...
export function flowerSVG(date,count,{large=false,animate=false,calendar=false}={}){
  ...
  const closeups={ume:'14 7 72 72',tulip:'18 12 64 64',nemophila:'14 7 72 72',hydrangea:'17 8 68 70','morning-glory':'17 6 70 70',osmanthus:'15 7 60 60'};
  const viewBox=calendar?(closeups[flower.id]||'10 3 80 80'):'0 0 100 108';
  return `<svg viewBox="${viewBox}" class="flower ${calendar?'calendar-flower':''} ${large?'large':''} ${animate?'bloom':''}" style="${palette}" data-flower="${flower.id}" data-progress="${progress}" role="img" aria-label="${flower.name}、${count}人と話しました、開花${Math.round(progress*100)}%">${flower.render(progress)}</svg>`;
}
